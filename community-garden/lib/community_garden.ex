# Use the Plot struct as it is provided
defmodule Plot do
  @enforce_keys [:plot_id, :registered_to]
  defstruct [:plot_id, :registered_to]
end

defmodule CommunityGarden do
  def start(), do: Agent.start(fn -> {[], 1} end)

  def list_registrations(pid), do: Agent.get(pid, fn {list, _} -> list end)

  def register(pid, register_to) do
    Agent.get_and_update(pid, fn {list, next_id} ->
      plot = %Plot{plot_id: next_id, registered_to: register_to}
      {plot, {[plot | list], next_id + 1}}
    end)
  end

  def release(pid, plot_id) do
    Agent.update(pid, fn {list, next_id} ->
      {Enum.filter(list, fn plot -> plot.plot_id != plot_id end), next_id}
    end)
  end

  def get_registration(pid, plot_id) do
    Agent.get(pid, fn {list, _} ->
      Enum.find(list, {:not_found, "plot is unregistered"}, fn plot -> plot.plot_id == plot_id end)
    end)
  end
end
